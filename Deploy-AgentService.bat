...
echo ============================================================
echo Deploy FunStudio Windows Maintenance Agent Service
echo Service: %SERVICE_NAME%
echo Binary : %AGENT_EXE%
echo ============================================================
echo.
...
if not "%ERRORLEVEL%"=="0" (
    ...
    echo Right-click Deploy-AgentService.bat and choose "Run as administrator".
    pause
    exit /b 1
)
...
if not exist "%AGENT_EXE%" (
    ...
    echo %AGENT_EXE%
    echo.
    echo Run Publish-Agent.bat first.
    pause
    exit /b 1
)
...
if "%ERRORLEVEL%"=="0" (
    echo Service exists. Stopping %SERVICE_NAME%...
    sc stop "%SERVICE_NAME%" >nul 2>&1
    timeout /t 3 /nobreak >nul

    echo Reconfiguring %SERVICE_NAME%...
